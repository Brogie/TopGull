...
namespace WindowsGame1
{
    ...
    class PowerUp : Sprite
    {
        ...
        static Random rand = new Random();//Random number for the Y axis spawn
        static Random time1 = new Random();//Random numbers for the respawn delay
        static Random time2 = new Random();//Random numbers for the respawn delay
        ...
        public PowerUp(int inScreenWidth, int inScreenHeight, Texture2D inSpriteTexture, int inDrawWidth, float inResetY, float inResetX, float inResetXSpeed) :
        ...
        public void RandomPlace()//spawn powerup in a random place
        {
            int x = screenWidth + rectangle.Width;
            int y = -100;
            SetPosition(x, y);
        }

        public void DelayRandomPlace()
        {
            if (time1.Next(100) == time2.Next(250))
            {
                int x = screenWidth - rectangle.Width;
        ...
        public override void Update(float deltaTime)
        {
            ...
            if (xPosition < 0) DelayRandomPlace();
    ...
    class Enemy : Sprite
    {
        ...
        public Enemy(int inScreenWidth, int inScreenHeight, Texture2D inSpriteTexture, int inDrawWidth, float inResetY, float inResetX, float inResetXSpeed) :
            base(inScreenWidth, inScreenHeight, inSpriteTexture, inDrawWidth, inResetX, inResetY)
        {
            resetXSpeed = inResetXSpeed;
            Reset();
        ...
        public void RandomEnemyPlace()//spawn enemy in a random place
        {
            int x = screenWidth + rectangle.Width;
            int y = -250;
            SetPosition(x, y);
        ...
        public void DelayRandomEnemyPlace()
        {
            if (time1.Next(100) == time2.Next(300))
            {
                int x = screenWidth - rectangle.Width;
                int y = rand.Next(0, screenHeight - rectangle.Height + 250);
                SetPosition(x, y);
            }
        ...
        public override void Update(float deltaTime)
        {
            ...
            if (xPosition < 0) DelayRandomEnemyPlace();
            if (xPosition + rectangle.Width > screenWidth) xPosition = screenWidth - rectangle.Width;
    ...
    class Mover : Sprite
    {
        ...
        public override void Update(float deltaTime)
        {
            ...
            {
                xPosition = (xPosition - (xSpeed * deltaTime) / 1.5f);
                yPosition = (yPosition - (ySpeed * deltaTime) / 1.5f); 
            }
            ...
            if (MovingLeft && (!MovingUp&&!MovingDown)) xPosition = xPosition - (xSpeed * deltaTime);
            if (MovingRight && (!MovingUp && !MovingDown)) xPosition = xPosition + (xSpeed * deltaTime);
            if (MovingUp && (!MovingLeft && !MovingRight)) yPosition = yPosition - (ySpeed * deltaTime);
            if (MovingDown && (!MovingLeft && !MovingRight)) yPosition = yPosition + (ySpeed * deltaTime);

            if (xPosition < 0) xPosition = 0;
            if (xPosition + rectangle.Width > screenWidth) xPosition = screenWidth - rectangle.Width;

            if (yPosition < 65) yPosition = 65;
            if (yPosition + rectangle.Height > screenHeight) yPosition = screenHeight - rectangle.Height;
    ...
    class PhysicsMover : Mover
    {
        ...
        public override void Update(float deltaTime)
        {
            ...
            if (yPosition < 64) ySpeed = Math.Abs(ySpeed);
