...
namespace WindowsGame1
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        List<PhysicsMover> bombs = new List<PhysicsMover>();
        List<GamePadState> gamePads = new List<GamePadState>();
        List<PowerUp> powerups = new List<PowerUp>();
        List<Enemy> pidgeons = new List<Enemy>();
        ...
        void gameOver()
        {
            ...
            foreach (PhysicsMover p in bombs)
            {
                p.Reset();
            }
            if (round == 3)
        ...
        protected override void LoadContent()
        {
            ...
            Texture2D poweruptex = Content.Load<Texture2D>("Cloud");
            Texture2D enemytex = Content.Load<Texture2D>("pigeon");
            ...
            btnPlay.setPosition(new Vector2(550, 550));

            rain = new ParticleGenerator(Content.Load<Texture2D>("Rain"), graphics.GraphicsDevice.Viewport.Width, 200);
           ...
           // MediaPlayer. = true;
            ...
            header = new Sprite(screenWidth, screenHeight, headerTexture, 1300, 0, 0);
            //gameSprites.Add(header);

            int enemyWidth = screenWidth / 20;

            for (int i = 0; i < 1; i++)
            {
                bomb = new PhysicsMover(screenWidth, screenHeight, enemyTexture, enemyWidth, screenWidth / 2, screenHeight / 2, 1000 , 1000 , 1000 , 1000 , 0.6f);
                gameSprites.Add(bomb);
                bombs.Add(bomb);
            }
            ...
            int powerUpWidth = screenWidth / 7;
            int enemyWidth2 = screenWidth / 20;

            for (int i = 0; i < 1; i++)
            {
                oneup = new PowerUp(screenWidth, screenHeight, poweruptex, powerUpWidth, 0, screenWidth / 3.7f, screenWidth / 3.7f);
                gameSprites.Add(oneup);
                powerups.Add(oneup);
            }

            for (int i = 0; i < 1; i++)
            {
                pidgeon = new Enemy(screenWidth, screenHeight, enemytex, enemyWidth2, 0, screenWidth / 1.5f, screenWidth / 1.5f);
                gameSprites.Add(pidgeon);
                pidgeons.Add(pidgeon);
            }

            oneup.StartMovingLeft();
            pidgeon.StartMovingLeft();
        ...
        void updateEndRound(GameTime gameTime)
        {
            ...
            if(round == 3)
            {
                ...
                if (player1Wins < player2Wins)
                    messageString = "USSR won the game! ";
            ...
            else
            {
                ...
                if (player1Score < player2Score)
                    messageString = "USSR won round " + round;
                if (player1Score == player2Score)
                    messageString = "Round " + round + " was a draw";
            }
        ...
        void updateGamePlay(GameTime gameTime)
        {
            ...
            int secsLeft = timer / 60;
            messageString = "Germany: " + player1Score + "        Time: " + secsLeft.ToString() + "        USSR: " + player2Score;
